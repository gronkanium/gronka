<script>
  ...
  async function loadPrices() {
    try {
      ...
      if (prices.monero !== null && prices.monero !== moneroPrice) {
        moneroAnimating = false;
        await tick();
        moneroPriceKey += 1;
        moneroPrice = prices.monero;
        await tick();
        moneroAnimating = true;
        setTimeout(() => {
          moneroAnimating = false;
        }, 300);
      }
...
<section class="health">
  ...
  {:else if health}
    <dl>
      ...
      <div class="stat-item">
        <dt>
          <svg class="crypto-icon" viewBox="0 0 3756.09 3756.49" xmlns="http://www.w3.org/2000/svg">
        ...
        <dd class="price-container">
          <span class="price-roll" class:animating={moneroAnimating}>{formatPrice(moneroPrice)}</span>
        </dd>
      </div>
    </dl>
  {/if}
